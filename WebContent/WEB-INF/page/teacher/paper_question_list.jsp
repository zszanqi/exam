...
<body class="app sidebar-mini rtl">
            ...
            <table class="table table-hover">
              ...
              <tbody align="center">
              	<c:forEach items="${page.data }" var="paper" varStatus="status">
	                  ...
	                  <td id="status${paper.id }">${paper.status }
	                  ...
	                  <td id="sbtn${paper.id }">
	                  		<c:if test="${paper.fk_status == 1 }">
	                  			<button class="btn btn-secondary my_del" type="button" disabled="disabled">未初始化</button>
	                  		</c:if>
	                  		<c:if test="${paper.fk_status == 2 }">
	                  			<button class="btn btn-warning rowsCount = 0; my_del" onclick="setStatus('3','${paper.id}')" type="button">开始运行</button>
	                  		</c:if>
	                  		<c:if test="${paper.fk_status == 3 }">
	                  			<button class="btn btn-danger my_del" onclick="setStatus('4','${paper.id}')" type="button">立即停止</button>
	                  		</c:if>
	                  		<c:if test="${paper.fk_status == 4 }">
	                  			<button class="btn btn-success my_del" onclick="setStatus('3','${paper.id}')" type="button">重新运行</button>
	                  		</c:if>
	                  </td>
	                  <td>
					...
					success:function(data){
						if(data=='ok'){
							...
							if(status=='3'){
								...
								$("#sbtn"+paperId).html("<button class=\"btn btn-danger my_del\" onclick=\"setStatus('4','+paperId+')\" type=\"button\">立即停止</button>");
							}
							if(status=='4'){
								$("#status"+paperId).html("已结束");
								$("#sbtn"+paperId).html("<button class=\"btn btn-success my_del\" onclick=\"setStatus('4','+paperId+')\" type=\"button\">重新运行</button>");
							}
						}else{
							alert("设置失败");
						}
	...
	function toAdd(){
		window.location.href='${basePath}manager/ToAddPaperServlet';
	}
	</script>
</body>
</html>