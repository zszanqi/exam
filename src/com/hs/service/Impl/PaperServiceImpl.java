...
public class PaperServiceImpl implements PaperService {
	...
	//根据id获取paper对象
	@Override
	public List<Exam> getPaperById(int paperId) {
		List<Exam> paper=null;
		try {
			paper=pd.getTeacherByid(paperId);
		} catch (SQLException e) {
			e.printStackTrace();
		}
		return paper;
	...
	//根据id编辑试卷
	@Override
	public String editPaper(int paperId, String title, String time) {
		String result = null;
		int rows = 0;
		try {
			rows = pd.editPaper(paperId,title,time);
			if(rows==1) {
				result = "ok";
			}
		} catch (SQLException e) {
